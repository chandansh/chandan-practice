...
<h2>URL :<%=iframeSrc%></h2>
<%-- User : <%=themeDisplay.getUser()%><br> --%>
<%-- User Id : <%=themeDisplay.getUserId()%><br> --%>
<%-- Scope Group Id : <%=themeDisplay.getScopeGroupId()%><br> --%>
<%-- Scope Group Name : <%=themeDisplay.getScopeGroupName()%><br> --%>

<h2>Modified URL : <%=getmodifiedURL(iframeSrc, themeDisplay) %></h2>
<%

//out.print("Org Id"+orgId+" name"+compny.getAdminName());
//out.print("Company Name:"+compny.getName()+" Id :"+compny.getCompanyId());

%>
<%!
private String getmodifiedURL(String url , ThemeDisplay themeDisplay)throws Exception{
	...
	url=StringUtil.replace(url, "@@USER_ID@@", String.valueOf(user.getUserId()));
	url=StringUtil.replace(url, "@@USER_NAME@@", String.valueOf(user.getFullName()));
	url=StringUtil.replace(url, "@@ORG_ID@@", String.valueOf(orgId));
	url=StringUtil.replace(url, "@@ORG_NAME@@", orgName);
	return url;
}
