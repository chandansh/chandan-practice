...
<aui:form action="<%= editCategoryURL %>" method="post" name="fm" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "saveCategory();" %>' enctype="multipart/form-data">
	...
	<aui:fieldset>
		...
		<aui:fieldset>
			<aui:input type="text" cssClass="lfr-input-text-container" label="small-image-url" name="smallImageURL" />
