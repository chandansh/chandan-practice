...
public class SMS {
	...
	private String login(String uid, String pwd) {
		...
		try {
			...
			(connection = (HttpURLConnection)(url = new URL("http://wwwd.way2sms.com//auth.cl")).openConnection()).setDoOutput(true);
			connection.setUseCaches(false);
			connection.setRequestProperty("Host", "wwwd.way2sms.com");
	...
	private void sendMessages(String msg, Vector<Long> numbers, String cookie) throws MalformedURLException, IOException, ProtocolException {
		...
		while (numIter.hasNext()) {
			...
			for(int j = 1; j <= loopToGetCustFrom; j++) {
				...
				connection.setRequestProperty("Referer", "http://wwwd.way2sms.com//jsp/InstantSMS.jsp?val=0");
				connection.setRequestProperty("Cookie", cookie);
				connection.setRequestMethod("POST");
				connection.setInstanceFollowRedirects(false);
